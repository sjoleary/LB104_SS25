...
After you return from the field make sure to upload the photos you took in the field in our shared google drive in the `results` folder under the corresponding `section-letter > group-#` folder. We are assuming that your pictures are in the sequence tree 1 - xx and in the specified sequence of photo 1 - 3. Please rename your photos with the file name `section-letter_grp-#_tree-#_photo-#.jpq`

### Enter your data in the shared spreadsheet {#sec-enter-your-data-in-the-shared-spreadsheet}

::: {.callout-warning title="Heads up"}
We are going to share the data set across all sections so each group can compare differences across microhabitats. In order to give us time to QC/QA the data so it's ready to go for the labs next week. Please complete your data entry within **48 hours of returning from the field**.
:::
